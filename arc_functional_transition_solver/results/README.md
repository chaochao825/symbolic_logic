...
| `executable_abductive_workspace_v0_1_qa_20260812_attempt4_dependency_regression` | VERIFIED CONTROLLED TYPED-SKETCH INFRASTRUCTURE | Typed sketches, proof obligations, content identities, abstract diagnosis, fill actions, and legacy replay pass the frozen dependency regression. This is syntax/execution infrastructure, not natural ARC utility. |
| `executable_abductive_workspace_recolor_v0_2_qa_20260812_attempt2` | VERIFIED CONTROLLED TOPOLOGY-ACTUATOR PASS | One legal `Grid -> Grid` recolor insertion closes all three controlled positive loops, rejects five negative controls, and matches concrete enumeration in all 81 exhaustive checks. This validates the bounded mechanism only. |
| `executable_abductive_workspace_recolor_natural_v0_2_arc_tgi_dev50_20260812` | VERIFIED NATURAL-UTILITY NULL | On 196 exactly reconstructed scene parents, four parents from one task admit the recolor insertion, but their one exact output duplicates the incumbent. Novel frontier and unique recovery are both 0/50 versus equal-reserved-cost cold; the three-recovery promotion gate fails and no next node or router training is authorized. |
| `anchor_rasterized_delta_v0_3_arc_tgi_dev50_20260812` | VERIFIED SENSOR NULL | The finite anchor-mask topology passes 75 dependency tests and byte-identical freeze/score replay, but only 1/50 tasks passes strict LODO versus the preregistered 3/50 sensor threshold. Its exact output is already incumbent, global/whole-component recolor also explains the one reachable task, and novel frontier plus unique recovery are 0/50. The exposed cohort cannot be used to add another mask primitive. |
