"""CLI for the frozen anchor-rasterized-delta development gate."""
...
from afts_arc.anchor_mask_topology_gate import (  # noqa: E402
    SCIENTIFIC_LANES,
    freeze_anchor_mask_opportunities,
...
def _freeze(args: argparse.Namespace) -> dict[str, object]:
    ...
    source_paths = (
        ...
        PROJECT_ROOT / "src" / "afts_arc" / "anchor_mask.py",
        PROJECT_ROOT / "src" / "afts_arc" / "anchor_mask_reachability.py",
        PROJECT_ROOT / "src" / "afts_arc" / "anchor_mask_topology_gate.py",
    ...
    freeze = freeze_anchor_mask_opportunities(
...
def _score(args: argparse.Namespace) -> dict[str, object]:
    ...
    return {
        ...
        "mask_exact": result["mask_exact"],
        "novel_mask_exact": result["novel_mask_exact"],
...
def main() -> None:
    ...
    freeze.add_argument("--scientific-lane", choices=SCIENTIFIC_LANES, required=True)
