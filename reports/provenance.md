...
- Rate-reduction artifacts: `results/logic_discovery_results.csv` (125 rows), `results/rate_reduction_results.csv` (60 rows), and `results/rate_logic_metadata.json`; the 46.09-second full run started from clean commit `5cddc7f4bd4df878359e1e60a765cd0f4db22aa2` on host `hi-X640-G40`, Python 3.10.20/NumPy 1.25.2, with one BLAS/OpenMP thread through SSH profile 210.
...
- Previous QA: a fresh bundle clone of artifact commit `892ec4a1d1a3348bec14c2e5da93054e9546f729` on SSH profile 210/Python 3.8.10 passed the then-current 39 repository tests and reproduced the 1,974-row summary. The current update receives a separate final QA entry after its result commit.
