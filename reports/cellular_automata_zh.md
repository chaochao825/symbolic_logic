...
| sync `phi_sync`，严格 M=2N | 1.000 | 0.990 | 0.977 | 1 / 1 / 1（M≈2N） |

主要趋势和论文一致，但 `phi_sync` 在本轮“精确 2N”协议的大尺寸结果没有被四舍五入成完美复现。论文写的是 `M≈2N` 且统计“在 M 内达到”；本轮结果因此只支持“成功率对 horizon 敏感”，而不能篡改为 1.00 或用未归档的临时诊断代替主协议。
