...
`--mode smoke` is a shorter run where supported. The main experiments and the discrete-theory catalog were generated by `--mode full`; their environments, clean starting commits, and artifact hashes are recorded in `results/run_metadata.json`, `results/rate_logic_metadata.json`, and `results/discrete_theory_metadata.json`.
